...
fun EnterPhoneScreen(
    onResult: (code: String, number: String) -> Unit,
) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
        ...
        EventsFilledButton(
            enabled = numberState.text.length == PHONE_NUMBER_LENGTH,
            modifier = Modifier
                .padding(horizontal = EventsTheme.sizes.sizeX5)
                .fillMaxWidth(),
            onClick = {
                onResult(countryState.value.countryCode, numberState.text.toString())
            }) {
            Text("Continue")
        }
    }
